...
let AUTH_MODE = "unknown"; // "service_account" | "oauth"
...
export async function getAuthClient() {
  // デプロイ環境では、OAuthトークンは失効しやすい（invalid_grant）ため、
  // service account を優先できるようにしておく（スプレッドシート側で共有が必要）
  if (process.env.GOOGLE_SERVICE_ACCOUNT_JSON) {
    ...
    try {
      ...
      // Render等で private_key 内の \n が実際の改行になった場合のフォールバック
